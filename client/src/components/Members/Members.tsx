import { AlertDialog, AlertDialogBody, AlertDialogContent, AlertDialogFooter, AlertDialogHeader, AlertDialogOverlay, Avatar, AvatarGroup, Box, Button, Checkbox, Flex, useDisclosure } from "@chakra-ui/react"
...
const AddMembersToBoard: React.FC = () => {
    ...
    return <Box width="300px" maxHeight="500px" padding="8px 16px">
        {boardMembers.map(member => <Flex cursor="pointer" onClick={() => onSelectMember(member)} marginBottom="4px" justifyContent="space-between" alignItems="center"> 
...
const ConfirmMemberDialog: React.FC<MemberDialog> = ({
    ...
    const { loading: loadingAddMemberRequestResponse, response: boardWithRemovedMember, fetchData: createBoardMember } = useAxios({
        method: 'post',
    ...
    const { loading: loadingRemoveMemberRequestResponse, response: boardWithNewMember, fetchData: removeBoardMember } = useAxios({
        method: 'put',
        url: ROUTES.ADD_REMOVE_MEMBER(orgId),
        data: {
            memberId,
          ...
          <AlertDialogOverlay>
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <Button 
                    isLoading={loadingAddMemberRequestResponse || loadingRemoveMemberRequestResponse}
