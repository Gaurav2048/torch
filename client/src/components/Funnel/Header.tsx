...
const Header: React.FC<OwnProps> = ({ column, tasks, index, createTask, openTask }) => {
            ...
            <Box zIndex={1000} top="0" position="sticky" bgColor="rgb(247, 247, 247)">
                <Box display="flex" marginBottom="18px" justifyContent="space-between" alignItems="center">
                    <Flex alignItems="center" gap="8px">
                        <Box width="8px" height="8px" borderRadius="50%" bgColor={COLORS[index]} />
                        <Text fontWeight="500" fontSize="md">{column.title}</Text>
                        <Box borderRadius="50%" fontWeight={400} bgColor="white" fontSize="12px" padding="2px 12px">{column.taskIds.length}</Box>
                    </Flex>
                    <Box {...provided.dragHandleProps} >
                        <BsThreeDotsVertical color="#333333" />
                    </Box>
                </Box>
                <Button onClick={handleCreateTask} marginBottom="12px" leftIcon={<GoPlus color={colorSchema.PRIMARY} />} size='md'  width="100%" color={colorSchema.PRIMARY}>Create New Task</Button>
            </Box>
