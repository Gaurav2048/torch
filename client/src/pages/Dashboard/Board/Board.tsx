...
const CreateBoard: React.FC = () => {
  ...
  useEffect(() => {
    if (response) {
        setOrg(produce(org, (draft: Organisation) =>{
                ...
                <FieldArray name="columns">
                    ...
                    <div>
                        <Flex gap="8px" marginTop="18px">
                            <Text fontSize="lg" fontWeight={600}>Columns</Text>
                            <AppButton type="button" size="sm" onClick={() => push({ _id: v4(), title: '' })}>
                                <CgAdd />
                            </AppButton>
                        </Flex>
                        ...
                        <Box key={index} alignItems="center">
                            <FormControl mt={4}>
                                <FormLabel htmlFor={`columns.${index}.title`}>Column Name</FormLabel>
                                <Flex gap="8px" alignItems="center">
                                    <Input size="sm" type="text" onChange={handleChange} id={`columns.${index}.title`} name={`columns.${index}.title`} />
                                    <AppButton type="button" size="sm" onClick={() => remove(index)}>
                                        <BsDash />
                                    </AppButton>
                                </Flex>
                                <ErrorMessage name={`columns.${index}.title`} component="div" />
                            </FormControl>
                        </Box>
                        ))}
                    </div>
