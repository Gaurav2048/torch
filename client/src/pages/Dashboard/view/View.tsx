...
const ViewTask: React.FC = () => {
    ...
    const { createComment, createReply, comments, fetchCommentLoader } = useComments()

    const boardMembers = useMemo<Array<Member>>(() => {
        return members.filter(member => board.members?.includes(member._id || '') )
    }, [members, board])
    ...
    return <Box height="70vh" overflow="hidden">
        ...
        <TaskDetails />
        {fetchCommentLoader ? <div>Loading...</div> : 
            <Comments 
                comments={comments || []} 
